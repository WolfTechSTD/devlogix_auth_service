...
from app.application.model.user import UpdateUserView, UserView
...
class UpdateUser(Interactor[UpdateUserView, UserView]):
    def __init__(
            self,
    ...
    async def __call__(self, data: UpdateUserView) -> UserView:
        if not await self.user_gateway.check_user_exists(cast(Id, data.id)):
